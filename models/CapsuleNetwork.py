...
from labml_helpers.train_valid import BatchIndex, SimpleTrainValidConfigs
from labml_nn.capsule_networks import MarginLoss, Router, Squash
...
batch_size_train = 64
batch_size_test = 128
weights_file = join(__root_folder, "models/checkpoint-capsule.pt")
...
class EMNISTConfigs(BaseConfigs):
    """
    ...
    train_batch_size: int = batch_size_train
    valid_batch_size: int = batch_size_test

    train_loader_shuffle: bool = True
    valid_loader_shuffle: bool = False


@option(EMNISTConfigs.dataset_transforms)
...
@option(EMNISTConfigs.train_dataset)
def mnist_train_dataset(c: EMNISTConfigs):
    return _dataset(True, c.dataset_transforms)


@option(EMNISTConfigs.valid_dataset)
def mnist_valid_dataset(c: EMNISTConfigs):
    return _dataset(False, c.dataset_transforms)


@option(EMNISTConfigs.train_loader)
def mnist_train_loader(c: EMNISTConfigs):
    return DataLoader(
        c.train_dataset, batch_size=c.train_batch_size, shuffle=c.train_loader_shuffle
    )


@option(EMNISTConfigs.valid_loader)
def mnist_valid_loader(c: EMNISTConfigs):
    return DataLoader(
        c.valid_dataset, batch_size=c.valid_batch_size, shuffle=c.valid_loader_shuffle
    )


aggregate(
    EMNISTConfigs.dataset_name,
    "MNIST",
    (EMNISTConfigs.dataset_transforms, "emnist_transforms"),
    (EMNISTConfigs.train_dataset, "emnist_train_dataset"),
    (EMNISTConfigs.valid_dataset, "emnist_valid_dataset"),
    (EMNISTConfigs.train_loader, "emnist_train_loader"),
    (EMNISTConfigs.valid_loader, "emnist_valid_loader"),
)


class EMNISTCapsuleNetworkModel(Module):
    """
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(
            in_channels=256, out_channels=32 * 8, kernel_size=9, stride=2, padding=0
        )
        self.squash = Squash()
        ...
        self.digit_capsules = Router(32 * 6 * 6, 47, 8, 16, 3)
    ...
    def forward(self, data: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            mask = torch.eye(47, device=data.device)[pred]
...
class Configs(EMNISTConfigs, SimpleTrainValidConfigs):
    """
    ...
    margin_loss = MarginLoss(n_labels=47)
    ...
    def step(self, batch: Any, batch_idx: BatchIndex):
        ...
        loss = self.margin_loss(caps, target) + 0.0005 * self.reconstruction_loss(
...
def capsule_network_model(c: Configs):
    """Set the model"""
    return EMNISTCapsuleNetworkModel().to(c.device)
...
def train():
    ...
    Run the experiment
    """
    experiment.create(name="capsule_network_emnist")
    conf = Configs()
    experiment.configs(
        conf, {"optimizer.optimizer": "Adam", "optimizer.learning_rate": 1e-3}
    )
...
def get_model() -> EMNISTCapsuleNetworkModel:
    if isfile(weights_file):
        model = EMNISTCapsuleNetworkModel()
        model.load_state_dict(torch.load(weights_file, weights_only=True), strict=False)
        return model
    else:
        train()
